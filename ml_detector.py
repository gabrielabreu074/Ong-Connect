...
os.environ["TRANSFORMERS_OFFLINE"] = "1"  # evita chamadas de rede em runtime
...
def _carregar_modelo():
    ...
    with _LOCK:
        if _MODELO_NLP is not None:
            return _MODELO_NLP
        print("[ML] carregando modelo...")
        from sentence_transformers import SentenceTransformer
        _MODELO_NLP = SentenceTransformer(
            "paraphrase-multilingual-MiniLM-L12-v2",
            device="cpu",
        )
        # Coloca em modo inferência e libera memória de treino
        _MODELO_NLP.eval()
        import torch
...
def _carregar_embeddings():
    ...
    if _EMB_GENUINAS is not None:
        return
    with _LOCK:
        if _EMB_GENUINAS is not None:
            return
        print("[ML] carregando embeddings...")
        import torch
        modelo = _carregar_modelo()
        with torch.no_grad():
            _EMB_GENUINAS = modelo.encode(MENSAGENS_GENUINAS, convert_to_tensor=True)
            _EMB_RUINS    = modelo.encode(MENSAGENS_RUINS,    convert_to_tensor=True)
...
def _penalidades(dados: dict) -> tuple[float, list[str]]:
    ...
    if not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", email):
        alertas.append("e-mail com formato inválido"); penalidade += 0.25
    if dominio in DOMINIOS_DESCARTAVEIS:
        alertas.append("e-mail descartável/temporário"); penalidade += 0.30
    if re.fullmatch(r"[a-z]{6,}[0-9]{4,}@.+", local):
        alertas.append("e-mail com padrão suspeito"); penalidade += 0.15
    ...
    if len(tel) not in (10, 11):
        alertas.append("telefone com tamanho inválido"); penalidade += 0.20
    if ddd and ddd not in DDDS_VALIDOS:
        alertas.append("DDD inexistente no Brasil"); penalidade += 0.20
    if numero and len(set(numero)) == 1:
        alertas.append("telefone com dígitos todos iguais"); penalidade += 0.15
    if numero in ("12345678", "123456789", "987654321"):
        alertas.append("telefone sequencial óbvio"); penalidade += 0.15

    nome = _normalizar(dados.get("nome", ""))
    if nome in NOMES_SUSPEITOS:
        alertas.append("nome suspeito ou genérico"); penalidade += 0.25
    if not re.search(r"\s", nome):
        alertas.append("nome sem sobrenome"); penalidade += 0.10
    if re.search(r"\d", nome):
        alertas.append("nome contém números"); penalidade += 0.15
